  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F7F8FA", display: "flex" }}>
      <div style={{ width: 240, background: NAVY, minHeight: "100vh", padding: "32px 20px", flexShrink: 0 }}>
        ...
        {steps.map((s, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 0", opacity: i > step ? 0.4 : 1 }}>
            <div style={{ width: 28, height: 28, borderRadius: "50%", background: i < step ? "#2ECC71" : i === step ? AMBER : "#252547", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: i < step ? "#fff" : NAVY, flexShrink: 0 }}>{i < step ? "✓" : i + 1}</div>
      ...
      <div style={{ flex: 1, padding: "48px", overflowY: "auto" }}>
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          ...
          {step === 1 && <StepTemplate data={data} setData={setData} />}
          {step === 2 && <StepBranding data={data} setData={setData} />}
          {step === 3 && <StepBusiness data={data} setData={setData} />}
          {step === 4 && <StepIntegrations data={data} setData={setData} />}
...
function StepCategory({ data, setData }) {
  return (
    <div>
      <h2 style={{ fontFamily: "'Sora',sans-serif", fontWeight: 700, fontSize: 36, color: NAVY, margin: "0 0 8px" }}>What type of business are you?</h2>
      <p style={{ color: "#6B7280", fontSize: 16, margin: "0 0 36px" }}>We'll recommend the best templates and integrations for you.</p>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16 }}>
        {categories.map((c, i) => {
          const selected = data.category === c.name;
          return (
            <div key={i} onClick={() => setData(d => ({ ...d, category: c.name }))} style={{ background: selected ? "#FFF8ED" : "#fff", border: `2px solid ${selected ? AMBER : "#E5E7EB"}`, borderRadius: 12, padding: 24, cursor: "pointer", transition: "all 0.15s", position: "relative" }}>
              ...
              <div style={{ fontFamily: "'Sora',sans-serif", fontWeight: 600, fontSize: 16, color: NAVY, marginBottom: 4 }}>{c.name}</div>
...
function StepTemplate({ data, setData }) {
  return (
    <div>
      <h2 style={{ fontFamily: "'Sora',sans-serif", fontWeight: 700, fontSize: 36, color: NAVY, margin: "0 0 8px" }}>Choose your template</h2>
      <p style={{ color: "#6B7280", fontSize: 16, margin: "0 0 36px" }}>Pick the look and feel for your app. You can customise everything next.</p>
        ...
        {templates.map((t, i) => {
          ...
          return (
            <div key={i} style={{ background: "#fff", border: `2px solid ${selected ? AMBER : "#E5E7EB"}`, borderRadius: 12, overflow: "hidden", cursor: "pointer", transition: "all 0.15s", position: "relative" }} onClick={() => setData(d => ({ ...d, template: t.name }))}>
              ...
              <div style={{ height: 180, background: NAVY, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 72 }}>{t.preview}</div>
      ...
      <div>
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            {colors.map((c, i) => (
              <div key={i} onClick={() => setData(d => ({ ...d, color: c }))} style={{ width: 40, height: 40, background: c, borderRadius: "50%", cursor: "pointer", border: data.color === c ? `3px solid ${AMBER}` : "3px solid transparent", outline: data.color === c ? `2px solid ${AMBER}` : "none", outlineOffset: 2, display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.15s" }}>
...
function StepBusiness({ data, setData }) {
  return (
        ...
        <div style={{ background: "#fff", border: "1px solid #E5E7EB", borderRadius: 8, overflow: "hidden" }}>
          {["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"].map((day, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 16, padding: "12px 16px", borderBottom: i < 6 ? "1px solid #F3F4F6" : "none" }}>
              <span style={{ width: 90, fontSize: 14, color: NAVY, fontWeight: 500 }}>{day}</span>
...
function StepIntegrations({ data, setData }) {
  ...
  return (
    <div>
      <h2 style={{ fontFamily: "'Sora',sans-serif", fontWeight: 700, fontSize: 36, color: NAVY, margin: "0 0 8px" }}>Supercharge your app</h2>
      <p style={{ color: "#6B7280", fontSize: 16, margin: "0 0 32px" }}>Pick the integrations that match your business. Add more anytime.</p>
...
function Published({ onFinish, showToast }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F7F8FA", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ textAlign: "center", maxWidth: 760, padding: 40 }}>
        ...
        <h1 style={{ fontFamily: "'Sora',sans-serif", fontWeight: 700, fontSize: 48, color: NAVY, marginBottom: 8 }}>Your App is Live!</h1>
        <p style={{ color: "#6B7280", fontSize: 18, marginBottom: 40 }}>Mama's Kitchen is now on DukaDesk. 🚀</p>
        <div style={{ background: "#fff", borderRadius: 16, padding: 40, boxShadow: "0 4px 24px rgba(0,0,0,0.08)", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 40, textAlign: "left", marginBottom: 32 }}>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontFamily: "'Sora',sans-serif", fontWeight: 600, fontSize: 16, color: NAVY, marginBottom: 16 }}>Your QR Code</div>
            <div style={{ width: 160, height: 160, background: "#F3F4F6", borderRadius: 12, margin: "0 auto 16px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 80 }}>▣</div>
            <div style={{ fontSize: 13, color: "#9CA3AF", marginBottom: 16 }}>dukadesk.app/mamas-kitchen</div>
