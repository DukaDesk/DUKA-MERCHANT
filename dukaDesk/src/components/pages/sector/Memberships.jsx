...
import { BadgeCheck, Users, DollarSign, Search } from "lucide-react";
...
export default function Memberships() {
  ...
  const active = stats?.customers || 0;

  const statsCards = [
    { label: "Active Members", value: active, icon: BadgeCheck, color: AMBER },
    { label: "Pending Renewals", value: orders?.filter(o => o.status !== "Completed").length || 0, icon: Users, color: "#7C3AED" },
    { label: "Revenue (Month)", value: `₦${(stats?.revenue || 0).toLocaleString()}`, icon: DollarSign, color: "#0D9488" },
  ];

  if (loading) return <Loading message="Loading memberships..." />;
  ...
  return (
    <div style={{ animation: "fadeIn 0.35s ease" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        <div>
          <h2 style={{ fontFamily: "'Sora',sans-serif", fontWeight: 700, fontSize: isMobile ? 22 : 28, color: NAVY, margin: 0 }}>Memberships</h2>
          <p style={{ color: "#6B7280", fontSize: 14, marginTop: 4 }}>Recurring plans, subscribers &amp; expiry.</p>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#F3F4F6", borderRadius: 10, padding: "8px 12px" }}>
          <Search size={16} color="#9CA3AF" />
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search member..." style={{ background: "none", border: "none", outline: "none", fontSize: 13, color: NAVY, width: 180, fontFamily: "inherit" }} />
        </div>
      </div>
          ...
          filtered.slice(0, 12).map((o, i) => {
            ...
            return (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 0", borderBottom: i < Math.min(filtered.length, 12) - 1 ? "1px solid #F3F4F6" : "none" }}>
                <div style={{ width: 36, height: 36, background: `${AMBER}14`, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: NAVY }}>{(o.customer || "M")[0]}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: NAVY }}>{o.customer || "Member"}</div>
                  <div style={{ fontSize: 12, color: "#6B7280" }}>{o.date || "Recent"} · {o.items || "Plan"}</div>
                </div>
                <span style={{ background: paid ? "#F0FDF4" : "#FFF8ED", color: paid ? "#065F46" : "#92400E", fontSize: 12, fontWeight: 600, padding: "4px 10px", borderRadius: 20 }}>{paid ? "Active" : "Expired"}</span>
              </div>
            );
          })
        )}
      ...
      <p style={{ fontSize: 12, color: "#9CA3AF", marginTop: 16 }}>Initial view bound to orders &amp; dashboard statistics. Recurring billing and plan builder arrive in a later phase.</p>
    </div>
  );
}